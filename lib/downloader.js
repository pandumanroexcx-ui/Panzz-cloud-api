...
async function downloadYoutube(url) {
  ...
  return {
    ...
    imageUrl: null,
    // YouTube audio aslinya M4A. Coba MP3 biar WhatsApp terima.
...
async function downloadDouyin(url) {
  ...
  try {
    ...
    console.log('[DOUYIN] btch gagal:', e.message);
  }
  ...
  try {
    ...
    console.log('[DEBUG douyin rahad]', JSON.stringify(result));
    const vid = firstString(
      result?.video, result?.url, result?.videoUrl,
      result?.data?.video, result?.data?.url,
      result?.result?.video, result?.result?.url
    );
    if (vid) return { videoUrl: vid, audioUrl: null, imageUrl: null };
  } catch (e) {
    console.log('[DOUYIN] rahad gagal:', e.message);
  }
