...
async function downloadInstagram(url) {
  ...
  // Deteksi dari URL yang dikirim user
  const isReel = /\/reel\//i.test(url) || /\/reels\//i.test(url);
  const isPost = /\/p\//i.test(url);

  // Deteksi dari URL media (kadang ke-mask jadi .webp, tapi ada hints lain)
  const urlLower = mediaUrl.toLowerCase();
  const isVideoByExt = /\.(mp4|mov|webm)(\?|$)/i.test(urlLower) || urlLower.includes('video');
  const isImageByExt = /\.(jpg|jpeg|png|webp)(\?|$)/i.test(urlLower);

  // Logika:
  // - Reel → selalu video
  // - Post dengan ekstensi video → video
  // - Post dengan ekstensi image → gambar
  // - Post gak jelas → cek parameter 'dl=1' (kalau video, biasanya ada 'filename=...mp4')
  let isVideo = isReel || (isPost && isVideoByExt);
  let isImage = false;

  // Kalau post gak jelas, cek token (biasanya ada 'filename=...' di base64 token)
  if (isPost && !isVideo && !isImageByExt) {
    const tokenMatch = mediaUrl.match(/filename["']?[:=]["']?([^"'&]+)/);
    if (tokenMatch && tokenMatch[1].match(/\.(mp4|mov|webm)/i)) {
      isVideo = true;
    } else {
      isImage = true;
    }
  } else if (isPost && isImageByExt) {
    isImage = true;
  }
  ...
  return {
    ...
    audioUrl: isVideo ? mediaUrl : null,
    audioFromVideo: isVideo,
  };
}
...
async function downloadCapcut(url) {
  ...
  if (!vid) throw new Error('CapCut lagi error.');
  return { videoUrl: vid, audioUrl: null, imageUrl: result.coverUrl || null };
}
...
async function downloadDouyin(url) {
  ...
  if (!links.length) throw new Error('Server Douyin lagi down, coba lagi nanti.');
