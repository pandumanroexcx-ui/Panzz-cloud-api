...
async function downloadYoutube(url) {
  ...
  return {
    ...
    imageUrl: null,
    // Info tambahan: YouTube audio aslinya M4A (audio/mp4)
    audioMime: 'audio/mp4',
    audioExt: 'm4a',
  };
}
...
async function downloadDouyin(url) {
  ...
  const links = result?.result?.data?.links || [];
  if (!links.length) throw new Error('Ga bisa ambil video dari Douyin ini.');
  ...
  return {
    ...
    audioUrl: audio || null,
    imageUrl: result?.result?.data?.thumbnail || null,
  };
}
