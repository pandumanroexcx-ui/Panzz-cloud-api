...
const { downloadTwitter } = require('./twitter');
...
async function downloadSpotify(url) {
  ...
  console.log('[DEBUG spotify]', JSON.stringify(result));
  const data = result?.result || result;
  const audio = firstString(data?.audio, data?.url, data?.download, data?.mp3);
  if (!audio) throw new Error('Ga bisa ambil audio Spotify.');
  return {
    audioUrl: audio,
    videoUrl: null,
    imageUrl: data?.thumbnail || null,
...
async function download(platform, url) {
  switch (platform) {
    ...
    case 'spotify': return downloadSpotify(url);
    default:
