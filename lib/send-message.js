...
// Tandai pesan user udah dibaca (centang biru)
async function markAsRead(messageId) {
  try {
    ...
    const data = await res.json();
    if (data.error) {
      console.error('[READ] gagal:', data.error.message);
    } else {
      console.log('[READ] ✓ centang biru terkirim');
    }
    return data;
  } catch (e) {
...
// Munculin "sedang menulis..." (typing indicator)
async function sendTyping(to, messageId) {
  try {
    // WhatsApp butuh baca pesan dulu, baru bisa tampil typing
    const url = `https://graph.facebook.com/v21.0/${PHONE_NUMBER_ID}/messages`;
    const res = await fetch(url, {
      method: 'POST',
      headers: {
    ...
    const data = await res.json();
    if (data.error) {
      console.error('[TYPING] gagal:', data.error.message);
    }
    return data;
  } catch (e) {
