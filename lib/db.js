...
try {
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_wl_user ON watchlists(user_id, status);
  `);
  console.log('[DB] Tabel watchlists siap');
} catch (e) {
  console.error('[DB] Gagal bikin tabel watchlists:', e.message);
}
...
try {
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_celengan_user ON celengan(user_id);
  `);
  console.log('[DB] Tabel celengan siap');
} catch (e) {
  console.error('[DB] Gagal bikin tabel celengan:', e.message);
}
