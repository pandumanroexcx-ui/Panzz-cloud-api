...
async function sendMessage(to, body, options = {}) {
  ...
  if (options.reply_to) payload.context = { message_id: options.reply_to };
...
async function sendDocument(to, mediaId, filename = 'document.pdf', caption = '') {
  return sendRaw({
...
async function markAsRead(messageId) {
  try {
    ...
    const data = await res.json();
    if (!data.error) console.log('[READ] ✓');
    return data;
...
module.exports = {
  sendMessage, sendImage, sendVideo, sendAudio, sendSticker, sendDocument, sendButtons, sendList,
