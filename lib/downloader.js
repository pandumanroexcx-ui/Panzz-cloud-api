...
async function downloadInstagram(url) {
  ...
  // IG post /p/ → biasanya gambar ATAU video
  // IG reel /reel/ → biasanya video
  const isReel = /\/reel\//i.test(url) || /\/reels\//i.test(url);
  const isPost = /\/p\//i.test(url);

  // Cek ekstensi (kadang ke-mask jadi .webp dari rapidcdn)
  const isVideoByExt = /\.(mp4|mov|webm)(\?|$)/i.test(mediaUrl);
  const isImageByExt = /\.(jpg|jpeg|png|webp)(\?|$)/i.test(mediaUrl);

  // Kalau reel → pasti video. Kalau post → cek ekstensi
  let isVideo = isReel || isVideoByExt;
  let isImage = !isReel && (isImageByExt || (!isVideoByExt && isPost));

  // Kalau post tapi gak jelas, asumsi gambar
  if (isPost && !isVideo && !isImage) isImage = true;
